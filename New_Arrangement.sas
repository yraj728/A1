...
***cumulative sum of the each customer card(cc)**;
data sale;
input cc merchant $ amnt;
cards;
1 N 100
...
proc sql;
select country,count(*) as cnt from sasuser.empdata where country in ("USA" "CANADA") group by country
...
proc sql;
select*, monotonic () as seq_no from sasuser.admit having seq_no between 10 and 17;
quit;

proc sql;
select*, monotonic () as seq_no from sasuser.admit having Age between 30 and 50; *sequencing done after the having condition applied;
quit;
...
proc sql;
select name,sortedby from dictionary.columns where lowcase(libname)="sasuser" and lowcase(memname)="admit";
quit;

proc sql;
select name,sortedby from dictionary.columns where lowcase(libname)="sasuser" and lowcase(memname)="admit" and sortedby gt 0;
quit;
